...
const Rabbits = () => {
  ...
  const [allRabbits, setAllRabbits] = useState<Pet[]>([]);
  const [rabbits, setRabbits] = useState<Pet[]>([]);
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Find your gentle companion. Browse through our collection of adorable rabbits waiting for a loving home.
            </p>
          </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-8">
              {rabbits.map((rabbit) => (
                <PetCard
                  ...
                  age={`${rabbit.age} year${rabbit.age !== 1 ? 's' : ''}`}
                  price={`₹${rabbit.price.toLocaleString()}`}
                  image={rabbit.image_url || "/placeholder.svg"}
                  gender={rabbit.gender || "Unknown"}
                  onAdopt={() => handleAdopt(rabbit)}
                />
              ))}
            </div>
          )}
